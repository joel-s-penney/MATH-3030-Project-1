...
We may first try analytical methods to determine $x$, which is often convenient if $f$ is sufficiently basic, like many polynomials of degree 1 or 2. Frequently, however, our function $f$ is more complicated or sophisticated, and analytical methods are impractical or unavailable. Instead, numerical methods are often imposed on \textit{root-finding problems} such as the one described. Root-finding algorithms (or root-finding methods) are used to approximate the roots or zeros of a function, with the requirement that the function be continuous. Additionally, solving the equation \[g(x)=h(x)\] is equivalent to finding the roots of $f(x)=g(x)-h(x)$, which shows how widespread root-finding problems are and how important root-finding methods can be. 
...
\begin{theorem}[Fundamental Theorem of Algebra]
    Every nonzero polynomial of degree $n$ with complex coefficients has exactly $n$ complex roots when multiplicity is counted.
\end{theorem}
...
Now we explain the bisection algorithm. First, we check that the function values at the endpoints have opposite signs. If they have the same sign (that is, $f(a)\cdot f(b) >0$), then the method cannot be applied. Note that if $f(a)$ or $f(b)$ is zero, then a root is found. Otherwise, we calculate the midpoint $m$ of the interval by \[m=\frac{a+b}{2}\] and check if it is a root. If it is not, then we check if $f(m)$ has the same sign as $f(a)$, in which case $a$ is replaced by $m$. Else, $b$ is replaced by $m$. A new midpoint is then calculated and the process repeats.
...
\end{align*}
for the interval $-60 \le x_0 \le -10$ we have considered. On this interval, as the initial guess tends to the true root, the error decreases, which is expected. These results show that Newton's method is fairly useful and comparable to methods such as secant method in this context. Also, Halley's method is well-equipped to deal with starting guesses that are far away from the root, in this specific case. Remember, however, that open-methods are not always guaranteed to converge like bracketed methods are. As mentioned previously, specifics such as the inputs to the algorithms and the polynomial or function itself are all factors that affect which method will converge the fastest; none of these methods are superior in all contexts and they all have advantages and disadvantages.  

\section{Conclusion}
\noindent
By understanding how two famous root-finders work, we able to explain why the bisection method must converge to a root in an interval and why horizontal regions pose a threat to Newton's method. Our Python code allowed for visualization of the two main methods, and we used important concepts such as the Fundamental Theorem of Algebra to solve root-finding problems. Additionally, we extended our study to a total of six algorithms and observed promising results for methods like TOMS748 and Halley's method. Using Wilkinson's polynomial and a limited number of iterations, we tracked absolute error and remarked about its relationship to the initial approximation for open methods. Throughout, we observed that there is no usual ``best'' method, as each has the potential to outperform the others depending on the specifics of the problem and polynomial considered.

Future research can build upon these findings and explorations. Building on the Figure \ref{errorplot} approach, one could plot the absolute error, or perhaps the relative error, across more $x$-values and find curves that reveal more precisely how open methods deal with far initial guesses. Alternatively, it would be interesting to study root-finder success in the context of high multiplicity roots: are there more efficient ways to find even multiplicity roots specifically? What about high multiplicity roots next to each other? Trying functions other than polynomials in another clear next step. Developing new techniques or modifying existing algorithms could lead to very useful methods in some contexts.

Polynomial root-finding will remain an important topic as modelling and computing applications continue to grow. Clearly, we have demonstrated that the best suited root-finders (and supporting theory) will differ based on the polynomial of interest, the tolerance or error desired, and the initial parameters for each method.

\addcontentsline{toc}{section}{Acknowledgements}
\section*{Acknowledgements}
\noindent
I would like to thank Dr. Alex Bihlo for his feedback and guidance throughout the project.

The following is a declaration on the use of generative artificial intelligence. Generative AI was used on two parts of this project. First, ChatGPT was used to improve upon the plot-generating code and show the previous approximations with an appropriate legend. Second, generative AI was used to define Wilkinson's polynomial and its derivatives in the code, which was a better choice than hard-coding the polynomial. Both of these uses were checked to ensure accuracy. 
...
\section{Root-Finding Algorithms in Python}\label{manualalgorithms}
